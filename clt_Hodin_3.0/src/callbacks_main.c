...
void cb_files_downloader(GtkButton *button, gpointer user_data)
{
    ...
    GtkWidget *files_infos = NULL;
    GtkWidget *username_infos = NULL;
    ...
    files_infos = gtk_message_dialog_new(GTK_WINDOW(main_win), GTK_DIALOG_MODAL, GTK_MESSAGE_WARNING , GTK_BUTTONS_CLOSE, "With this option you can download scripts and text files.\n\nFor download videos, binaries, music and so on \n\nuse the option : Download Binary");
    ...
    username_infos = gtk_message_dialog_new(GTK_WINDOW(main_win), GTK_DIALOG_MODAL, GTK_MESSAGE_WARNING , GTK_BUTTONS_CLOSE, "The username can differ from your username.\n\nFor get the remote username use command : user\n\nfrom the remote shell.\n");

    gtk_widget_show_all(GTK_DIALOG(username_infos)->vbox);
    switch(gtk_dialog_run(GTK_DIALOG(username_infos)))
    {
        default :
            gtk_widget_destroy(username_infos);
            break;
    }

    downloader_dialog = gtk_dialog_new_with_buttons("Download File", GTK_WINDOW(main_win),  GTK_DIALOG_MODAL, GTK_STOCK_APPLY, GTK_RESPONSE_APPLY, NULL);
    gtk_widget_set_size_request(downloader_dialog, 460, 150);

    downloader_entry = gtk_entry_new();
    gtk_entry_set_text(GTK_ENTRY(downloader_entry), "path + filename : example -> /home/username/Desktop/filename");
    ...
    gtk_widget_set_size_request(progress_bar_text, 130, 20);
    ...
    switch(gtk_dialog_run(GTK_DIALOG(downloader_dialog)))
    {
        case GTK_RESPONSE_APPLY:
            ...
            download_files(file_path, downloader_dialog);
...
void cb_get_file_path_save(GtkWidget *button, GtkWidget *file_selection)
{
    ...
    FILE *fichier = NULL;
    GtkWidget *file_saved_editor = NULL;
    ...
    file_saved_editor = gtk_message_dialog_new (GTK_WINDOW(main_win), GTK_DIALOG_MODAL, GTK_MESSAGE_INFO, GTK_BUTTONS_NONE, "File saved !");
    switch(gtk_dialog_run(GTK_DIALOG(file_saved_editor)))
    {
        default :
            gtk_widget_destroy(file_saved_editor);
            break;
    }
...
//  Open the hosts file in a window
...
void cb_files_uploader(GtkButton *button, gpointer user_data)
{
    ...
    gtk_widget_set_size_request(progress_bar_upload, 130, 20);
