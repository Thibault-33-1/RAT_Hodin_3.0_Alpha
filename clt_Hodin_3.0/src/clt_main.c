...
void fill_window(int argc, char *argv[])
{
    ...
    */
    ...
    /* zone[1] = Remote shell */
    gtk_fixed_put(GTK_FIXED(zone[1]), image_decoration[2], 550, 470);
    gtk_fixed_put(GTK_FIXED(zone[1]), image_decoration[3], 540, 500);

    /* zone[2] = DDOS */
    gtk_fixed_put(GTK_FIXED(zone[2]), image_decoration[4], 760, 160);
    gtk_fixed_put(GTK_FIXED(zone[2]), image_decoration[5], 830, 215);
...
\tcomputer across a computer network.\"\n\n\t\t\t\t\t\t\t\t\t[ Wikipedia ]", -1);
    ...
    utf8_indentity = g_locale_to_utf8("<span foreground=\"#ffffff\">Identity : </span>\n", -1, NULL, NULL, NULL);
    utf8_country = g_locale_to_utf8("<span foreground=\"#ffffff\">Country : </span>\n", -1, NULL, NULL, NULL);
